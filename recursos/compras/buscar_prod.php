...
if(isset($_GET["term"]))
{
    ...
    $result = $conexion->query("SELECT a.id, a.linea, a.descripcion, a.foto, a.combo, c.cantidad,(SELECT f.descuento FROM detalle_compra f WHERE f.codp = a.id ORDER BY f.codc DESC LIMIT 1) AS descuento, (SELECT d.pupesos FROM inventario d WHERE d.fecha_reg = (SELECT MAX(e.fecha_reg) FROM inventario e WHERE e.codp = a.id) AND d.codp = a.id AND CONCAT(d.codp,' ',a.descripcion) LIKE '%".$_GET["term"]."%' LIMIT 1) AS pupesos, b.nombre FROM productos a, lineas b, invcant c WHERE a.id = c.codp AND a.linea = b.codli AND a.combo = 0 AND a.estado = 1 AND CONCAT(a.id,' ',a.descripcion) LIKE '%".$_GET["term"]."%' ORDER BY id ASC;");
    ...
    if($total_row > 0){
      foreach($result as $row)
      {
       ...
       $temp_array['cantidad'] = $row['cantidad'];
